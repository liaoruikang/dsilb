...
const publish = async (id: string) => {
  const pkg = await readPackageJSON(id).catch(() => null);
  if (!pkg) return Promise.reject(`Package ${basename(id)} not found\n`);
...
await Promise.all(targets.map(target => publish(target))).finally(() => {
  if (!skipBuild) targets.map(target => rimraf(join(target, 'dist')));
});
